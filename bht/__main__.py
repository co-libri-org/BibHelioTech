    ...
    for folders_or_pdf in os.listdir(_base_pdf_dir):
        ...
        for pdf_files in os.listdir(pdf_paths):  # processing treatment.
            if pdf_files.endswith(".pdf"):
                print(pdf_paths)
                if len(os.listdir(
                        pdf_paths)) == 1:  # Only 1 file (the pdf) --> directory never treated --> processing first treatment.
                    ...
                    ocr_filter(pdf_paths)  # filter result of the OCR to deletes references, change HHmm4 to HH:mm, etc ...
                    SUTime_treatement(pdf_paths,
                                      sutime)  # SUTime read all the file and save its results in a file "res_sutime.json"
                    SUTime_transform(
                        pdf_paths)  # transforms some results of sutime to complete missing, etc ... save results in "res_sutime_2.json"
                    ...
                    ocr_filter(pdf_paths)  # filter result of the OCR to deletes references, change HHmm to HH:mm, etc ...
                    SUTime_treatement(pdf_paths,
                                      sutime)  # SUTime read all the file and save its results in a file "res_sutime.json"
                    SUTime_transform(
                        pdf_paths)  # transforms some results of sutime to complete missing, etc ... save results in "res_sutime_2.json"
