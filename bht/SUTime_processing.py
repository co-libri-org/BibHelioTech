    ...
    for dicts in test_list:
        try:
            ...
            if re.search('PRESENT_REF', str(dicts['value'])):  # remove times of type "..._REF"
                ...
                dicts.clear()
            elif re.search('^([^0-9]*)$', str(dicts[
                                                  'text'])):  # remove times that do not contains digit (like 'today', 'dusk', 'the night', etc...)
                dicts.clear()
...
def nearest_date(JSON_list, compteur_dicts):
    ...
    while compteur_avant >= 0:
        if JSON_list[compteur_avant]['type'] == "DATE":
            ...
            if re.search("((^[0-9]{4})-[0-9]{2}-[0-9]{2}$)", JSON_list[compteur_avant]['value']['begin']) and re.search(
                    "((^[0-9]{4})-[0-9]{2}-[0-9]{2}$)", JSON_list[compteur_avant]['value']['end']):
                avant = JSON_list[compteur_avant]
                break
        compteur_avant -= 1
    ...
    while compteur_apres < len(JSON_list):
        if JSON_list[compteur_apres]['type'] == "DATE":
            ...
            if re.search("((^[0-9]{4})-[0-9]{2}-[0-9]{2}$)", JSON_list[compteur_apres]['value']['begin']) and re.search(
                    "((^[0-9]{4})-[0-9]{2}-[0-9]{2}$)", JSON_list[compteur_apres]['value']['end']):
    ...
    elif abs(JSON_list[compteur_avant]['end'] - JSON_list[compteur_dicts]['start']) < abs(
            JSON_list[compteur_dicts]['end'] - JSON_list[compteur_apres][
                'start']):  # closer between before and after: it is before
        nearest = JSON_list[compteur_avant]
...
def nearest_year(JSON_list, compteur_dicts):
    ...
    if avant == 0:  # case: no DATE or DURATION before
        if JSON_list[compteur_apres]['type'] == "DATE":
            ...
            if re.search("((XXXX)-[0-9]{2}-[0-9]{2})", JSON_list[compteur_apres]['value']['begin']):
                ...
                if re.search("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_apres]['value']['begin']):
                    nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})",
                                       JSON_list[compteur_apres]['value']['begin']).group(2)
                elif re.search("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_apres]['value']['end']):
                    nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})",
                                       JSON_list[compteur_apres]['value']['end']).group(2)
    elif apres == 0:  # case: no DATE or DURATION after
        if JSON_list[compteur_avant]['type'] == "DATE":
            ...
            if re.search("((XXXX)-[0-9]{2}-[0-9]{2})", JSON_list[compteur_avant]['value']['begin']):
                ...
                if re.search("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_avant]['value']['begin']):
                    nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})",
                                       JSON_list[compteur_avant]['value']['begin']).group(2)
                elif re.search("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_avant]['value']['end']):
                    nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})",
                                       JSON_list[compteur_avant]['value']['end']).group(2)
    elif abs(JSON_list[compteur_avant]['end'] - JSON_list[compteur_dicts]['start']) < abs(
            JSON_list[compteur_dicts]['end'] - JSON_list[compteur_apres][
                'start']):  # plus proche entre avant et après: c'est avant
        if JSON_list[compteur_avant]['type'] == "DATE":
            nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_avant]['value']).group(2)
        elif JSON_list[compteur_avant]['type'] == "TIME":
            nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_avant]['value']).group(2)
            ...
            if matcher_begin != None:
                nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_avant]['value']['begin']).group(
                    2)
            matcher_end = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_avant]['value']['end'])
            if matcher_end != None:
        ...
        if JSON_list[compteur_apres]['type'] == "DATE":
            ...
            matcher_begin = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_apres]['value']['begin'])
            if matcher_begin != None:
                nearest = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_apres]['value']['begin']).group(
                    2)
            matcher_end = re.match("(([0-9]{4})-[0-9]{2}-[0-9]{2})", JSON_list[compteur_apres]['value']['end'])
            if matcher_end != None:
...
def SUTime_transform(current_OCR_folder):
        ...
        try:
            ...
            if dicts['type'] == "TIME":
                ...
                if test != None:
                    ...
                    while test.group(group_counter_max) == '' or test.group(
                            group_counter_max) == None:  # stop test at HH or MM or ...
                        group_counter_max -= 1
                    ...
                    if group_counter_max == 16:  # end of ms (milliseconds)
                        if group_counter_min == 1:  # beginning in the years
                            begin = ''.join(list(test.group(1, 2, 4, 5, 7, 8, 9, 10, 12, 13, 14, 15))) + str(
                                "%03d" % (int(test.group(group_counter_max)) - 1))
                            end = ''.join(list(test.group(1, 2, 4, 5, 7, 8, 9, 10, 12, 13, 14, 15))) + str(
                                "%03d" % (int(test.group(group_counter_max)) + 1))
                        elif group_counter_min == 4:  # beginning at months
                            begin = ''.join(list(test.group(4, 5, 7, 8, 9, 10, 12, 13, 14, 15))) + str(
                                "%03d" % (int(test.group(group_counter_max)) - 1))
                            end = ''.join(list(test.group(4, 5, 7, 8, 9, 10, 12, 13, 14, 15))) + str(
                                "%03d" % (int(test.group(group_counter_max)) + 1))
                        elif group_counter_min == 8:  # start at T (undated time)
                            begin = ''.join(list(test.group(8, 9, 10, 12, 13, 14, 15))) + str(
                                "%03d" % (int(test.group(group_counter_max)) - 1))
                            end = ''.join(list(test.group(8, 9, 10, 12, 13, 14, 15))) + str(
                                "%03d" % (int(test.group(group_counter_max)) + 1))
                    elif group_counter_max == 14:  # end to the seconds
                        if group_counter_min == 1:  # beginning in the years
                            begin = ''.join(list(test.group(1, 2, 4, 5, 7, 8, 9, 10, 12, 13, 14))) + str(
                                ".%03d" % (495))
                        ...
                        if group_counter_min == 1:  # beginning in the years
                            begin = ''.join(list(test.group(1, 2, 4, 5, 7, 8, 9, 10, 12))) + str(
                                ":" + "%02d" % (0)) + str(":" + "%02d" % (0))
                            end = ''.join(list(test.group(1, 2, 4, 5, 7, 8, 9, 10, 12))) + str(
                                ":" + "%02d" % (59)) + str(":" + "%02d" % (59))
                        elif group_counter_min == 4:  # beginning at months
                            begin = ''.join(list(test.group(4, 5, 7, 8, 9, 10, 12))) + str(":" + "%02d" % (0)) + str(
                                ":" + "%02d" % (0))
                            end = ''.join(list(test.group(4, 5, 7, 8, 9, 10, 12))) + str(":" + "%02d" % (59))
                        elif group_counter_min == 8:  # start at T (undated time)
                            begin = ''.join(list(test.group(8, 9, 10, 12))) + str(":" + "%02d" % (0)) + str(
                                ":" + "%02d" % (0))
                            end = ''.join(list(test.group(8, 9, 10, 12))) + str(":" + "%02d" % (59)) + str(
                                ":" + "%02d" % (59))
                ...
                if 'begin' in dicts['value'] and 'end' in dicts['value']:
                    if re.search("^T.*", dicts['value']['begin']) and re.search("^T.*", dicts['value']['end']):
                        ...
                        if nearest['type'] == "DATE":
                            temp = {'begin': nearest['value'] + dicts['value']['begin'],
                                    'end': nearest['value'] + dicts['value']['end']}
                            dicts['value'] = temp
                        elif nearest['type'] == "DURATION":
                            temp = {'begin': nearest['value']['begin'] + dicts['value']['begin'],
                                    'end': nearest['value']['end'] + dicts['value']['end']}
                            dicts['value'] = temp
                    elif re.search("^T.*", dicts['value']['begin']) and re.search(
                            "(([0-9]{4})(-)([0-9]{2})(-)([0-9]{2})).*",
                            dicts['value']['end']):  # case date in one but not in the other
                        temp = {
                            'begin': re.match("(([0-9]{4})(-)([0-9]{2})(-)([0-9]{2})).*", dicts['value']['end']).group(
                                1) + dicts['value']['begin'], 'end': dicts['value']['end']}
                        dicts['value'] = temp
                    elif re.search("^T.*", dicts['value']['end']) and re.search(
                            "(([0-9]{4})(-)([0-9]{2})(-)([0-9]{2})).*",
                            dicts['value']['begin']):  # case date in one but not in the other
                        temp = {'begin': dicts['value']['begin'],
                                'end': re.match("(([0-9]{4})(-)([0-9]{2})(-)([0-9]{2})).*",
                                                dicts['value']['begin']).group(1) + dicts['value']['end']}
                        dicts['value'] = temp
    ...
    for dicts in JSON_list:
        if dicts['type'] != "DURATION":
            ...
            if re.search("[0-9]{2}-[0-9]{2}$", dicts['value']['begin']) and re.search("[0-9]{2}-[0-9]{2}$",
                                                                                      dicts['value']['end']):
    ...
    for dicts in JSON_list:
        if not re.search('^([0-9]{4})(-)([0-9]{2})(-)([0-9]{2})(T)([0-9]{2})(:)([0-9]{2})(:)([0-9]{2})(.)([0-9]{3})$',
                         dicts['value']['begin']) or not re.search(
    ...
    file = open(current_OCR_folder + "/" + 'res_sutime_2.json',
                'w')  # save the transformed results in a separate file. This is the file that will be read for later linking of intervals/sat/inst/etc.
